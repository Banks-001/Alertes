...
function buildData() {
  ...
  ALL = raw.map((d, i) => {
    ...
    const days = dUntil(dlD);
    const key  = d.t + '|' + d.dl;
    return { id:i, key, title:d.t, arrete:d.a||'—', deadline:d.dl, period:d.p, entity:d.e, dlD, days, st:getSt(days), custom:i>=csvData.length, transmitted:transmittedSet.has(key) };
...
function sendEmailAlerts(docs){
  ...
  console.group('[RégAlert] Alertes email');console.log('Destinataires:',notifEmails);
  docs.forEach(d=>console.log(`ALERTE: ${d.title} | ${d.entity} | ${fmtS(d.deadline)} | ${dLabel(d.days,d.st)}`));
  console.groupEnd();
}

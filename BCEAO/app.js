...
function processExcelFile(file) {
  ...
  reader.onload = function(e) {
    try {
      ...
      if (rows.length < 2) { showCsvStatus('error', '❌ Le fichier Excel est vide ou ne contient pas de données.'); return; }
      // Convertir en texte CSV pour réutiliser processCSV
      ...
      showCsvStatus('error', '❌ Erreur de lecture Excel : ' + err.message);
    }
  };
...
function downloadExcelTemplate() {
  // Créer un classeur Excel simple via SheetJS
...
// ══════════════════════════════════════════════════════
// IMPORT CSV (moteur interne, appelé par Excel)
// ══════════════════════════════════════════════════════
/*
...
// ══════════════════════════════════════════════════════
// EMAILJS — Configuration & Envoi réel
// ══════════════════════════════════════════════════════
let ejsConfig = { publicKey: '', serviceId: '', templateId: '' };
let _ejsInitialized = false;
let emailSentLog = {}; // { docKey: 'YYYY-MM-DD' }

function initEmailJS() {
  if (!ejsConfig.publicKey || _ejsInitialized) return;
  try { emailjs.init({ publicKey: ejsConfig.publicKey }); _ejsInitialized = true; }
...
async function loadEmailJSConfig() {
  ...
  try {
    const db = window._firebaseDb, uid = currentUser.uid;
    const doc = await window._firestoreGetDoc(window._firestoreDoc(db,'users',uid,'data','emailjs'));
    if (doc.exists()) {
      const d = doc.data();
      if (d.config)       ejsConfig    = { ...ejsConfig, ...d.config };
      if (d.emailSentLog) emailSentLog = d.emailSentLog;
      initEmailJS();
    }
  } catch(e) { console.warn('[EmailJS] Chargement:', e); }
  renderEmailJSConfig();
}
...
async function saveEmailJSConfig() {
  ...
  if (!pk || !sid || !tid) { showEjsStatus('error','⚠️ Veuillez remplir les 3 champs.'); return; }
  ejsConfig = { publicKey:pk, serviceId:sid, templateId:tid };
  _ejsInitialized = false; initEmailJS();
  if (!currentUser) { showEjsStatus('error','⚠️ Non connecté.'); return; }
  try {
    await window._firestoreSetDoc(
      window._firestoreDoc(window._firebaseDb,'users',currentUser.uid,'data','emailjs'),
      { config:ejsConfig, emailSentLog }, { merge:true }
    );
    showEjsStatus('success','✅ Configuration sauvegardée.');
...
function renderEmailJSConfig() {
  ...
  const sid=document.getElementById('ejs-service-id');
  const tid=document.getElementById('ejs-template-id');
  if(pk)  pk.value  = ejsConfig.publicKey  || '';
  if(sid) sid.value = ejsConfig.serviceId  || '';
  if(tid) tid.value = ejsConfig.templateId || '';
}

function showEjsStatus(type, msg) {
  const el = document.getElementById('ejs-status'); if(!el) return;
  el.style.display  = '';
  el.style.background = type==='success'?'var(--green-lt)':type==='loading'?'var(--blue-lt)':'var(--red-lt)';
  el.style.color      = type==='success'?'var(--green)':type==='loading'?'var(--blue)':'var(--red)';
  el.style.border     = '1px solid '+(type==='success'?'rgba(26,138,74,0.2)':type==='loading'?'rgba(26,106,191,0.2)':'rgba(217,48,37,0.2)');
  el.textContent = msg;
}
...
async function testEmailJS() {
  ...
  showEjsStatus('loading','⏳ Envoi du mail test…');
  initEmailJS();
  try {
    await emailjs.send(ejsConfig.serviceId, ejsConfig.templateId, {
      to_email   : notifEmails.join(', '),
      to_name    : 'Équipe BCEAO',
      subject    : '🔔 RégAlert — Test de notification',
      message    : 'Ceci est un email de test envoyé depuis RégAlert BCEAO. La configuration fonctionne correctement.',
      date_envoi : new Date().toLocaleDateString('fr-FR',{weekday:'long',day:'2-digit',month:'long',year:'numeric',hour:'2-digit',minute:'2-digit'})
    });
    showEjsStatus('success','✅ Email test envoyé vers : '+notifEmails.join(', '));
...
async function sendEmailAlerts(docs) {
  ...
  if (toSend.length===0) { console.log('[RégAlert] Emails déjà envoyés aujourd\'hui.'); return; }
  initEmailJS();
  const lines = toSend.map(d =>
    (d.days<=settings.urgentDays?'🔴':'🟡')+' '+d.title+' | '+d.entity+' | Échéance : '+fmtS(d.deadline)+' | '+dLabel(d.days,d.st)
  ).join('\n');
  try {
    await emailjs.send(ejsConfig.serviceId, ejsConfig.templateId, {
      to_email   : notifEmails.join(', '),
      to_name    : 'Équipe BCEAO',
      subject    : '🔔 RégAlert — '+toSend.length+' échéance'+(toSend.length>1?'s':'')+' à traiter',
      message    : 'Bonjour,\n\nVoici les échéances réglementaires BCEAO nécessitant votre attention :\n\n'+lines+'\n\nCes rappels s\'arrêteront dès que vous cliquerez sur "Transmis" dans l\'application.\n\nCordialement,\nRégAlert BCEAO',
      date_envoi : new Date().toLocaleDateString('fr-FR',{weekday:'long',day:'2-digit',month:'long',year:'numeric',hour:'2-digit',minute:'2-digit'})
    });
    toSend.forEach(d => { emailSentLog[d.key] = today; });
    if (currentUser) {
      window._firestoreSetDoc(window._firestoreDoc(window._firebaseDb,'users',currentUser.uid,'data','emailjs'),{config:ejsConfig,emailSentLog},{merge:true}).catch(()=>{});
    }
    console.log('[RégAlert] ✅ Email envoyé à',notifEmails.join(', '));
  } catch(e) { console.error('[RégAlert] ❌ Échec email:',e?.text||e?.message||e); }
}
...
function emptyState(msg=''){
  return`<div class="empty" style="grid-column:1/-1"><div class="e-ico">📂</div><h3>Aucune donnée importée</h3><p>${msg||'Importez un fichier <strong>Excel</strong> depuis <strong>⚙️ Paramètres</strong>.'}</p></div>`;
}
function renderDash(){
  updateKPIs();
  // Uniquement les urgents NON transmis
  ...
  el.innerHTML = urgents.length === 0
    ? '<div class="empty" style="grid-column:1/-1"><div class="e-ico">✅</div><h3>Aucune échéance urgente en attente</h3><p>Tous les documents urgents ont été transmis.</p></div>'
    : urgents.map(makeCard).join('');
}
